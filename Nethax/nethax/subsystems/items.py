...
def apply_blessing(state: ItemEffects) -> ItemEffects:
    """Set BUC status to BLESSED.

    No-op stub (mkobj.c: bless / prayer reward path).
    Wave 3: set blessed=True, cursed=False, update parent Item.buc_status.
    """
    return state


def apply_curse(state: ItemEffects) -> ItemEffects:
    """Set BUC status to CURSED.

    No-op stub (mkobj.c: curse).
    Wave 3: set cursed=True, blessed=False, update parent Item.buc_status.
    """
    return state


def erode(state: ItemEffects, kind: int) -> ItemEffects:
    """Apply one step of erosion of the given kind.

    kind should be one of the Erosion enum families (RUSTY/BURNT/CORRODED/ROTTED).
    No-op stub (obj.h oeroded/oeroded2 bitfields; do_wear.c ARM_BONUS).
    Wave 3: increment eroded or eroded2, clamp to 3, respect erodeproof flag.
    """
    return state


def enchant(state: ItemEffects, delta: int) -> ItemEffects:
    """Apply an enchantment delta to an item.

    No-op stub (read.c: doenchant / enchant_armor / enchant_weapon).
    Wave 3: clamp enchantment per item category, handle over-enchant destruction.
    """
    return state
